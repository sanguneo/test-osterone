...
test("maskDynamic preserves amounts and IDs while masking declared dynamic formats", () => {
	...
	expect(masked).toContain("1234567");
});
...
test("gate: dynamic-only change still matches (masking); a real change drifts", () => {
	...
	s.approve("C1", 1, "staging");
	expect(s.gate("C1", 1, "staging", "Order 100001 at 2026-07-18T11:22").status).toBe("match");
	expect(s.gate("C1", 1, "staging", "Order 999999 at 2026-07-18T11:22").status).toBe("drift");
