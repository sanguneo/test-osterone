...
모델 연결은 **전역**이며, **톱바**에 있습니다 — 로그인형 컨트롤(●상태+모델명)을 누르면 세 가지 모드의 모달이 열립니다: **Codex 로그인**(로컬 `codex login` 자동 감지), **토큰 직접입력**(+모델 오버라이드), 또는 **API Key / 엔드포인트** — model + Base URL로 *임의의* OpenAI 호환 엔드포인트(Azure OpenAI, OpenRouter, Together, 로컬 vLLM/Ollama)에 연결합니다. 모델은 항상 **작성 시점**에만 쓰이고 판정에는 절대 개입하지 않습니다.

톱바 아래 네비게이션은 **2-lane 구조**입니다: 프로젝트 lane과 테스트 시트 lane이 각각 **바로 추가/수정/삭제**를 지원합니다(프로젝트 편집기 모달과 시트 편집기 모달 — 별도 "관리" 화면 없음). **프로젝트**는 하나 이상의 1급 **테스트 시트**(구글 시트 URL / CSV 붙여넣기 / `.xlsx` 업로드)를 담고, 공유 기본값(대상 URL, 환경, 테스트 계정, 참고 repo, AI 토글)을 갖습니다. 각 **테스트 시트**는 대상 URL·환경·열 매핑을 독립적으로 **오버라이드**할 수 있습니다. **시트 개수 캡은 없습니다** — 프로젝트나 시트 목록이 8개를 넘으면 검색/필터가 나타나고, 활성 항목은 자동으로 스크롤되어 보이며, 사이드바는 독립 스크롤이 되는 스티키 상태를 유지하고, 좁은 화면에서는 lane이 반응형으로 바뀌며, 긴 이름에는 툴팁이 붙습니다.

- **시트별 런타임** — 모든 시트는 **자신만의 실행 히스토리와 리뷰 큐**를 가집니다. 대시보드는 프로젝트 내 모든 시트를 집계하고, 리뷰 네비 배지는 프로젝트 단위 롤업을 보여줍니다. 시트를 실행하면 그 시트만 인제스트됩니다(시트별 dedupe).
- **AI 시트 해석 (열 매핑)** — **시트 해석**이 시트 헤더 + 샘플 행을 연결된 모델에 보내 열 매핑(`id/title/step/expected/priority/…` → 실제 헤더명)을 제안하고, 이를 **선택된 시트 위에 delta로** 기록합니다 — 프로젝트 공유 규칙은 그대로 유지됩니다. 대화로 조정할 수 있습니다("소분류 말고 중분류를 제목으로"). 이 매핑은 해당 시트의 **인제스트에 실제로 적용**됩니다.
...
**대화형 규칙 다듬기.** 연결 후, **AI 규칙 다듬기**로 프로젝트 단위 해석 규칙을 자연어로 다듬습니다(예: "누르기도 click으로 인식해"). **대화식**이라 이전 턴을 이어받고("그건 되돌려") 매 턴마다 **intent diff**와 **모호·빈 intent 경고**를 보여줘, 규칙이 최적의·해석가능한 형태로 수렴합니다. 변경은 규칙 버전을 올리며, **초기화**로 대화를 리셋합니다.
...
**리뷰 큐.** `needs_review` 케이스가 **스크린샷**·페이지 텍스트·사유(self-heal, baseline 미승인 등) 증거와 함께 뜹니다. baseline을 한 번 승인하면 매치되는 재실행은 같은 케이스 콘텐츠를 공유하는 **모든 시트에서** **통과**하고(reconcile-on-read가 재실행 없이 다른 시트의 stale needs_review를 정리), 페이지가 바뀌면 다시 표시됩니다. 신뢰 모델의 human-in-the-loop 그대로 — 애매한 소수만 한 번 승인하면 이후 자동이며, 조용한 거짓 통과는 없습니다.

**영속.** 프로젝트 메타데이터는 `~/.test-osterone/studio-projects.json`에, 프로젝트별 런타임 상태(규칙, refine 대화, 플랜 캐시, 승인된 baseline, 시트별 히스토리+리뷰 큐)는 `~/.test-osterone/studio-state/<projectId>.json`에 저장됩니다. 그리고 **시트 CSV 본문은 시트별 파일로 오프로드**되어(`sheet-data/<projectId>/<sheetId>.csv`) 두 파일 모두 시트 개수와 무관하게 작게 유지됩니다 — 그래서 캡이 없습니다. `baselineKey`/`assertionCacheKey` 포맷은 그대로이므로 false-pass=0이 전 구간에서 유지됩니다.
...
**구축·검증 완료 (정적·결정적 — 자동화 테스트 87/87):** 입력 → 정규화 → 중복제거 → 규칙(CLI) → 선별 → 해석 → assertion 캐시 → 실행 → 판정 → baseline → 증거 → 러너 계약 · 벤치마크 하드 게이트 · 웹 대시보드 · 오케스트레이션(노드/호스트) · 인증(API key + OAuth 프록시) + JUnit.
