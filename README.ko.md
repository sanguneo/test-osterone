...
**모델 연결(선택).** **Codex 로그인**을 누르면 로컬 Codex/ChatGPT 로그인을 그대로 재사용합니다(OAuth 프록시 — 토큰·모델을 `~/.codex`에서 읽음). 액세스 토큰이나 API 키를 직접 넣어도 됩니다. 연결하면 **AI 규칙 다듬기**로 해석 규칙을 자연어로 다듬을 수 있습니다(예: "누르기도 click으로 인식해"). 모델이 규칙을 갱신하면 규칙 버전이 올라가고 이후 실행에 반영됩니다.
