...
UI는 워크플로 순서의 번호 탭으로 구성됩니다 — **1 프로젝트 → 2 모델 연결 → 3 AI 규칙·해석 → 4 실행 & 결과 → 5 리뷰 큐**:

- **프로젝트** — 재사용 프로젝트를 저장합니다: **이름, 소스(샘플 / 구글 시트 / CSV 붙여넣기), 대상 사이트 URL, 환경, 테스트 계정(아이디/비밀번호), 참고 repo, 기본 AI 토글**. `~/.test-osterone/studio-projects.json`에 영속되고, 번들 **샘플** 프로젝트는 항상 제공됩니다. 계정·참고 repo는 AI 스텝 해석에 컨텍스트로 전달됩니다(예: 로그인 스텝 작성).
- **TC 읽기 & 중복 확인** — 프로젝트의 시트/CSV를 현재 열 매핑으로 인제스트해, 감지된 열·구조화된 케이스·**중복 제거된 케이스**를 실행 전에 보여줍니다.
- **실행** — 프로젝트를 고르고 필요하면 **AI 스텝 해석**을 켠 뒤 **실행**을 누릅니다.
