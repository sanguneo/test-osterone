			...
			<div className="card">
				...
				{mode === "token" && (
					<>
						...
						<label>모델 <span className="muted">— 선택 (기본 gpt-5.6-sol)</span></label>
						<input type="text" value={model} onChange={(e) => setModel(e.target.value)} placeholder="gpt-5.6-sol" autoComplete="off" />
					</>
				)}
				{mode === "apikey" && (
					<>
						<label>API Key</label>
						<input type="password" value={apiKey} onChange={(e) => setApiKey(e.target.value)} placeholder="sk-… / 임의 키" autoComplete="off" />
						<label>모델 <span className="muted">— 선택 (기본 gpt-4o-mini)</span></label>
						<input type="text" value={model} onChange={(e) => setModel(e.target.value)} placeholder="gpt-4o-mini / llama3.1 / …" autoComplete="off" />
						<label>Base URL <span className="muted">— 선택 · OpenAI 호환 엔드포인트</span></label>
						<input type="text" value={baseUrl} onChange={(e) => setBaseUrl(e.target.value)} placeholder="https://api.openai.com/v1" autoComplete="off" />
