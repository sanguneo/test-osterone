...
export function RunPanel({ project, selId, selSheetId, onDone }: { readonly project?: Project; readonly selId: string; readonly selSheetId: string; readonly onDone: () => void }) {
	...
	async function startRun() {
		...
		try {
			await api.runStream({ sample: project.id === "sample", sheets: sheet ? [sheet] : [], sheetId: sheet?.id, aiInterpret: ai, headed, baseUrl: project.baseUrl, env: project.env, accounts: project.accounts, referenceRepo: project.referenceRepo, lenientMatch: project.lenientMatch, projectId: selId }, (event) => {
	...
	async function startRunAll() {
		...
		push();
		try {
			await api.runAllStream({ sample: false, sheets, aiInterpret: ai, headed, baseUrl: project.baseUrl, env: project.env, accounts: project.accounts, referenceRepo: project.referenceRepo, lenientMatch: project.lenientMatch, projectId: selId }, (event) => {
				if (controller.signal.aborted) return;
