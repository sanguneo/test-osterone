...
bun test               # 320/320 (for contributors)
```
...
- **Step recovery ladder** — a step that misses the live DOM is not shrugged off. The engine (1) snaps a drifted label onto the one the page actually carries; (2) clears whatever intercepts input (modal/notice popup), lets the app settle and **retries once**; (3) with a model connected, **intervenes mid-run**: it re-reads the live screen (structural scan + screenshot) and asks for a corrected action, accepting it **only if the label/route it names actually exists on that screen** (budgeted, 2 per case); (4) if the step still cannot run, **stops the case** instead of replaying the rest of the plan against a screen the plan no longer describes — no stray clicks on the wrong page. An absence is confirmed on a screen that has stopped painting, so a slow SPA mount is never mistaken for a missing control. Every rung is recorded as a heal event, so a recovered case is still capped at `needs_review` (false-pass stays 0) and the review panel explains exactly what was repaired or skipped. A step the rule engine cannot interpret is likewise recorded rather than silently skipped.
- **The same ladder for the precondition** — the setup a case's `사전조건` describes gets the rungs its own steps get, with its own repair budget of 1 so a rescued setup cannot spend what the case's failing step needs. A setup that cannot finish holds the case as **precondition unmet** rather than failing it: nothing about the app was tested, so no verdict is owed. A repair here may also say it comes **before** the failed action rather than instead of it — the control a setup needs is often inside a closed menu and the trigger is what is on screen. The engine performs the trigger and then **retries the original action**, and only that retry counts as progress; booking the trigger as the setup step is how a case ends up running on a screen its precondition never reached. The model does not have to say so: if the control that could not be found is on screen *after* the repair, the screen has answered the question.
...
**Built & verified** (static, deterministic — 320/320 automated tests):
...
src/
  ...
  cli.ts · index.ts
test/           unit + smoke suites (320/320)
